...
export default class Downloader extends Component {
	...
	handleClickDown = (e) => {
		e.preventDefault();
		const { exports, action, param,method } = this.props;
		if (exports) {
			var ExportForm = document.createElement("FORM");
			document.body.appendChild(ExportForm);
			ExportForm.method = method||'GET';
	...
	render() {
		...
		return (
			renderNode == 'a' ?
				<span  onClick={this.handleClickDown} style={{ ...style, display: 'inline-block' }}>{title}</span>
				:
